...
from .trt_llm.tensorrt_llm_run import generate, load, load_refit
from .trt_llm.tensorrt_llm_build import is_engine_loaded
...
class TensorRTLLM(ITritonDeployable):
    ...
    def build(
        ...
        from .trt_llm.nemo_utils import WrappedNemoSentencePiece

        if self.tokenizer is None:
            self.tokenizer = WrappedNemoSentencePiece(nemo_model.tokenizer)

        model_dir = self.model_dir
        if parallel_state.get_data_parallel_world_size() > 1:
             model_dir = os.path.join(
                model_dir, f"dp{parallel_state.get_data_parallel_rank()}")
        if parallel_state.get_tensor_model_parallel_world_size() > 1:
             model_dir = os.path.join(
                model_dir, f"tp{parallel_state.get_tensor_model_parallel_rank()}")
        if parallel_state.get_pipeline_model_parallel_world_size() > 1:
            model_dir = os.path.join(
                model_dir, f"pp{parallel_state.get_pipeline_model_parallel_rank()}")
